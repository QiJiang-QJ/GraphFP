...
derive_dPhi_dW <- function(p,Phi,W,u1,a_dKL_dp2,lambda_i,beta){
  ...
  D3 <- sign(A2F)
  ADA <- (t(B)) %*% (D1 * B)
  ADDA <- (t(B)) %*% (as.numeric(D2 * D3) * B)
  A2Fp1 <- Time_array_array(A2F,p)
  A2Fp2 <- Time_array_array(abs(A2F),p)
  ula <- (u1 -lambda_i*a_dKL_dp2) %>% as.numeric()
  dPhi <- as.numeric(as.numeric(A2F * D1) %*% B)+as.numeric(as.numeric(abs(A2F) * D2) %*% B)+as.numeric(ula %*% (ADA + ADDA))
  
  ADDAFp1 <- t(B) %*% (D1 * A2Fp1)
  ADDAFp2 <- t(B) %*% (D2 * A2Fp2)
  dW1 <- (ADDAFp1 + ADDAFp2)
